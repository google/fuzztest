...
impl ExecutionMode {
    ...
    pub fn from_fuzztest_options(options: &FuzzTestOptions) -> ExecutionMode {
        ...
        if options.fuzz_for.is_some() || options.jobs.is_some() {
            let fuzz_for = options.fuzz_for.unwrap_or(Duration::Indefinitely);
...
impl std::fmt::Display for Duration {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            Duration::Indefinitely => write!(f, "inf"),
            Duration::Fixed(duration) => write!(f, "{duration}"),
        }
    }
}
...
mod tests {
    ...
    fn test_duration_from_str_inf() {
        let duration: Duration = "inf".parse().expect("failed to parse 'inf'");
        expect_that!(duration, eq(Duration::Indefinitely));
    }

    #[gtest]
    fn test_duration_from_str_infinite() {
        let duration: Duration = "infinite".parse().expect("failed to parse 'infinite'");
        expect_that!(duration, eq(Duration::Indefinitely));
    }

    #[gtest]
    fn test_duration_from_str_fixed() {
        let duration: Duration = "10s".parse().expect("failed to parse '10s'");
        let expected_fixed = humantime::Duration::from(std::time::Duration::from_secs(10));
        expect_that!(duration, eq(Duration::Fixed(expected_fixed)));
    }

    #[gtest]
    fn test_duration_from_str_invalid() {
        let result: Result<Duration, _> = "invalid_duration".parse();
        expect_true!(result.is_err());
    }

    #[gtest]
    fn test_duration_display() {
        expect_that!(Duration::Indefinitely.to_string(), eq("inf"));
        let fixed = Duration::Fixed(humantime::Duration::from(std::time::Duration::from_secs(10)));
    ...
    fn test_jobs_options_parsing_env() {
        ...
        match ExecutionMode::from_fuzztest_options(&options) {
            ExecutionMode::Fuzz(fuzz_opts) => {
                ...
                expect_that!(fuzz_opts.fuzz_for, eq(Duration::Indefinitely));
            }
    ...
    fn test_replay_corpus_for_inf_env_succeeds() {
        ...
        expect_that!(options.replay_corpus_for, eq(Some(Duration::Indefinitely)));
        expect_that!(options.corpus_db.as_deref(), eq(Some("/tmp/corpus_db")));
    }

    #[gtest]
    fn test_replay_corpus_for_infinite_env_succeeds() {
        // SAFETY: Testing environment parsing in single-threaded context.
        unsafe {
            std::env::set_var("FUZZTEST_REPLAY_CORPUS_FOR", "infinite");
            std::env::set_var("FUZZTEST_CORPUS_DB", "/tmp/corpus_db");
        }
        ...
        let options = result.expect(
            "parsing should succeed when replay_corpus_for is infinite and corpus_db is present",
        );
        expect_that!(options.replay_corpus_for, eq(Some(Duration::Indefinitely)));
        expect_that!(options.corpus_db.as_deref(), eq(Some("/tmp/corpus_db")));
    }
