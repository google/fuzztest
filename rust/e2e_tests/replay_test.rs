...
fn replay_by_id_reproduces_panic(fixture: &EnvVars) {
    ...
    let crashing_value: i32 = 10;
    let serialized_input =
        postcard::to_stdvec(&crashing_value).expect("Failed to serialize value using postcard");
    std::fs::write(crash_file_dir.join(crash_id), &serialized_input)
...
fn replay_all_reproduces_all_failures(fixture: &EnvVars) {
    ...
    let serialized_input =
        postcard::to_stdvec(&crashing_value).expect("Failed to serialize value using postcard");
    std::fs::write(crash_file_dir.join("crash_1"), &serialized_input)
        .expect("Failed to write crash_1 file to db");

    let crashing_value: i32 = 20;
    let serialized_input =
        postcard::to_stdvec(&crashing_value).expect("Failed to serialize value using postcard");
    std::fs::write(crash_file_dir.join("crash_2"), &serialized_input)
        .expect("Failed to write crash_2 file to db");

    let crashing_value: i32 = 30;
    let serialized_input =
        postcard::to_stdvec(&crashing_value).expect("Failed to serialize value using postcard");
    std::fs::write(crash_file_dir.join("crash_3"), &serialized_input)
