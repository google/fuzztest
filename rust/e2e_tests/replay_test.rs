...
fn get_target_binary_path(fixture: &EnvVars) -> PathBuf {
    std::env::var_os("CARGO_BIN_EXE_replay_fuzz_tests_bin").map(PathBuf::from).unwrap_or_else(
        || {
            fixture
                .target_binary_path
                .parent()
...
fn replay_all_reproduces_all_failures(fixture: &EnvVars) {
    ...
    let centipede_args = [
        ...
        format!("--test_name={}", normalized_test_name),
        format!("--fuzztest_corpus_database={}", db_dir.display()),
        "--fuzztest_time_limit_per_test=5s".to_string(),
        "--exit_on_crash=0".to_string(),
    ];
    let centipede_args_refs: Vec<&str> = centipede_args.iter().map(|s| s.as_str()).collect();
    test_utils::run_centipede_with_args_expect_termination(fixture, &centipede_args_refs);
