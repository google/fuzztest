...
pub use fuzztest_options::{
    Duration, ExecutionMode, FuzzOptions, FuzzTestOptions, ReplayCorpusOptions, ReplayCrashOptions,
    TimeBudgetType,
};
            ...
            ExecutionMode::SmokeTest => {
                if self.test_path.is_some() {
                    ...
                    ExecutionMode::Fuzz(FuzzOptions {
                        fuzz_for: self.fuzztest_options.fuzz_for.unwrap_or(Duration::Indefinitely),
...
mod tests {
    ...
    fn test_build_run_command_replay_all_crashes() {
        ...
        assert!(envs.contains(&("FUZZTEST_REPLAY_FINDINGS".to_string(), Some("true".to_string()))));
        assert!(
            envs.contains(&("FUZZTEST_CORPUS_DB".to_string(), Some("/tmp/corpus_db".to_string())))
        );
        assert!(envs.contains(&(
    ...
    fn test_cli_option_parsing_replay_corpus_for_success() {
        ...
        assert_eq!(
            parsed.fuzztest_options.replay_corpus_for,
            Some("10s".parse().expect("valid duration string"))
        );
    ...
    fn test_cli_option_parsing_replay_corpus_for_inf() {
        ...
        assert_eq!(parsed.fuzztest_options.replay_corpus_for, Some(Duration::Indefinitely));
        assert_eq!(parsed.fuzztest_options.time_budget_type, TimeBudgetType::PerTest);
        assert_eq!(parsed.fuzztest_options.corpus_db.as_deref(), Some("/tmp/corpus_db"));
        ...
        assert_eq!(
            mode,
            ExecutionMode::ReplayCorpus(ReplayCorpusOptions {
                replay_corpus_for: Duration::Indefinitely,
                time_budget_type: TimeBudgetType::PerTest,
            })
        );
    }
    ...
    #[gtest]
    fn test_cli_option_parsing_replay_corpus_for_infinite() {
        let parsed = CargoFuzzTestOptions::try_parse_from([
            "cargo-fuzztest",
            "--replay-corpus-for",
            "infinite",
        ...
        .expect("valid replay-corpus-for infinite should parse successfully");
    ...
    fn test_build_run_command_replay_corpus_indefinite() {
        let options = CargoFuzzTestOptions {
            fuzztest_options: FuzzTestOptions {
                replay_corpus_for: Some(Duration::Indefinitely),
                time_budget_type: TimeBudgetType::Total,
                corpus_db: Some("/tmp/corpus_db".to_string()),
                ..Default::default()
            },
