...
static IS_TEST_EXECUTING: Mutex<Option<ExecuteContext>> = Mutex::new(None);
...
pub(crate) fn emit_finding(description: &str, signature: &str) {
    let guard = IS_TEST_EXECUTING.lock();
    let token = guard.as_ref().expect("emit_finding called when test is not executing");
    DIAGNOSTIC_SINK
        .lock()
        .as_ref()
        .expect("since the test is executing, the diagnostic sink will definitely be present")
...
pub(crate) fn try_emit_finding(description: &str, signature: &str) -> bool {
    if let Some(guard) = DIAGNOSTIC_SINK.try_lock() {
        if let Some(sink) = guard.as_ref() {
            if let Some(token) = IS_TEST_EXECUTING.lock().as_ref() {
...
// GenericCorpusValue (Box<dyn Any>) does not implement Clone.
// We clone by serializing and deserializing through the domain.
fn clone_corpus_value(
    domain: &dyn super::domains::GenericDomain,
    val: &GenericCorpusValue,
) -> anyhow::Result<GenericCorpusValue> {
    let serialized = domain.serialize_corpus(val)?;
    domain.parse_corpus(&serialized)
}
...
impl RustFuzzTestAdapter {
    ...
    pub fn mutate(&self, origin: &GenericCorpusValue, shrink: bool, sink: &mut InputSink) {
        let mut mutant = match clone_corpus_value(self.fuzz_test.domains(), origin) {
            Ok(val) => val,
            Err(e) => {
                emit_error(&format!("Failed to clone input for mutation: {:?}", e));
                return;
            }
        };
    ...
    pub fn execute(&self, input: &GenericCorpusValue, sink: &mut FeedbackSink) {
        ...
        // Store the execution context token in the global `IS_TEST_EXECUTING`
        // to allow reporting findings during this execution.
        // SAFETY: We are currently in the `execute` callback of `RustFuzzTestAdapter`.
        *IS_TEST_EXECUTING.lock() = Some(unsafe { ExecuteContext::new() });
...
pub fn run_smoke_test(fuzztest: &dyn FuzzTest) {
    ...
    let mut generic_corpus_value =
        fuzztest.domains().init(&mut rng).expect("failed to get initial corpus value");
    ...
    while start_time.elapsed() < smoke_test_duration {
        fuzztest
            ...
            .expect("failed to mutate corpus value");
        let result = fuzztest.execute(&generic_corpus_value);
        // DISCUSS: probably need a more "googletest" approach to handling the result here?
        assert!(result);
    }
    ...
    if let Some(status) = maybe_run_as_worker(&manager) {
        match status {
            ...
            WorkerStatus::Failure => {
                // DISCUSS: in this case, is there a better way to "exit" the unit test?
                panic!("FuzzTest worker reported failure")
            }
        }
    }
...
///
/// This function constructs a C-compatible [`engine_ffi::FuzzTestAdapterManager`] wrapping the given `manager`
/// with required callback function pointers (`get_binary_id_callback`, `get_test_name_callback`, `construct_adapter_callback`),
/// and invokes [`engine_ffi::FuzzTestWorkerMaybeRun`].
