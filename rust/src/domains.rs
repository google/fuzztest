...
pub trait GenericDomain {
    ...
    /// Clones a `corpus_value`.
    fn clone_corpus_value(&self, val: &GenericCorpusValue) -> anyhow::Result<GenericCorpusValue>;
}
    ...
    D::CorpusValue: 'static,
{
    ...
    fn clone_corpus_value(&self, val: &GenericCorpusValue) -> anyhow::Result<GenericCorpusValue> {
        let typed_val = val
            .downcast_ref::<D::CorpusValue>()
            .context("downcasting GenericCorpusValue for cloning")?;
        Ok(Box::new(typed_val.clone()))
    }
}
