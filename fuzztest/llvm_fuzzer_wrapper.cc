...
extern "C" size_t LLVMFuzzerMutate(uint8_t* data, size_t size,
  ...
  const fuzztest::MutationMetadata* mutation_metadata = nullptr;
  if (auto* coverage = fuzztest::internal::GetExecutionCoverage();
      coverage != nullptr) {
    mutation_metadata = &coverage->GetMutationMetadata();
  }
  domain.Mutate(val, bitgen, mutation_metadata, false);
  return val.size();
}
...
class ArbitraryByteVector
 ...
 public:
  ...
  void Mutate(corpus_type& val, absl::BitGenRef prng,
              const fuzztest::MutationMetadata* metadata,
