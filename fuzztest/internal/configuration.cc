...
std::string Configuration::Serialize() const {
  ...
  out.resize(
      ...
      SpaceFor(crashing_input_to_reproduce) + SpaceFor(replay_crash_attempts) +
      SpaceFor(replay_coverage_attempts) + SpaceFor(batch_triage_suspect_only) +
  ...
  offset = WriteIntegral(out, offset, batch_triage_suspect_only);
...
absl::StatusOr<Configuration> Configuration::Deserialize(
  ...
  return [=]() mutable -> absl::StatusOr<Configuration> {
    ...
    ASSIGN_OR_RETURN(batch_triage_suspect_only, Consume<bool>(serialized));
