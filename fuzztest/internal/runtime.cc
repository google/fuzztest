...
std::optional<std::string> GetReproductionCommand(
  ...
  auto command_template = configuration.reproduction_command_template;
  if (!command_template.has_value()) return std::nullopt;
  ...
  if (configuration.crashing_input_to_reproduce.has_value()) {
    return absl::StrReplaceAll(
        *command_template, {{kTestFilterPlaceholder,
...
void PrintReproductionInstructionsForUndeclaredOutputs(
    RawSink out, absl::string_view test_name, std::string& reproducer_path) {
  ...
  reproducer_path = absl::StrCat("/tmp/", kReproducerDirName, "/", file_name);
}
