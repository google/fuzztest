...
//
// The `(crash_type_data, crash_type_size)` slice passed to `callback` points
// either to a static string literal or into the `error_summary` buffer passed
// by the sanitizer runtime, and is valid for the duration of the callback
// invocation (or longer if the input `error_summary` outlives the call).
