...
// `should_stop` callback is provided, it will be periodically called (every
// 100ms): process would be terminated when it returns true.
TerminationStatus RunCommandWithCallbacks(
    ...
    std::optional<absl::FunctionRef<bool()>> should_stop = std::nullopt,
