...
// TypeErasedFuzzKeyT is the type of the key used to register a fuzzing
// function with the fuzzing bitgen.
using TypeErasedFuzzKeyT = const void*;

// Registers the fuzzing functions for Abseil distributions.
void RegisterAbslRandomFuzzingMocks(
    absl::FunctionRef<void(TypeErasedFuzzKeyT, TypeErasedFuzzFunctionT)>
        register_fn);
