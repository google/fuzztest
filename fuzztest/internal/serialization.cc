...
struct BinaryOutputVisitor {
  ...
  void operator()(const std::string& value) const {
    ...
    if (buf) {
      ...
      std::memcpy(buf + offset + 1, reinterpret_cast<const char*>(&size),
                  sizeof(size));
...
bool BinaryParseImpl(IRObject& obj, BinaryParseBuf& buf, int recursion_depth) {
  if (recursion_depth > kMaxParseRecursionDepth) return false;
  ...
  switch (h) {
    ...
    case BinaryFormatHeader::kObject: {
      ...
      for (uint64_t i = 0; i < vec_size; ++i) {
        if (!BinaryParseImpl(v.emplace_back(), buf, recursion_depth + 1))
          return false;
      }
      return true;
    }
...
bool IsInBinaryFormat(absl::string_view str) {
  return str.size() >= kBinaryHeader.size() &&
         std::memcmp(str.data(), kBinaryHeader.data(), kBinaryHeader.size()) ==
             0;
}
...
std::optional<IRObject> IRObject::FromString(absl::string_view str) {
  ...
  if (IsInBinaryFormat(str)) {
    ...
    if (!BinaryParseImpl(object, buf, /*recursion_depth=*/0) || !buf.empty())
      return std::nullopt;
  } else {
