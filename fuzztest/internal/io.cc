...
std::string WriteDataToDir(absl::string_view data, absl::string_view outdir) {
  std::string basename =
      absl::StrFormat("%016x", absl::Hash<absl::string_view>{}(data));
  const bool need_slash = !outdir.empty() && outdir.back() != '/';
  std::string filename = absl::StrCat(outdir, need_slash ? "/" : "", basename);
