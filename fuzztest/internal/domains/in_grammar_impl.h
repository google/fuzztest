...
class RegexLiteralDomain {
 public:
  ...
  static void Mutate(ASTNode& val, absl::BitGenRef prng,
    ...
    GetInnerRegexpDomain().Mutate(std::get<1>(val.children), prng, options);
  }
...
class TupleDomain {
 public:
  ...
  static void Mutate(ASTNode& val, absl::BitGenRef prng,
    ...
    ApplyIndex<sizeof...(ElementT)>([&](auto... I) {
      ((choice == I
            ? (ElementT::Mutate(std::get<2>(val.children)[I], prng, options))
            : (void)0),
       ...);
    });
  }
  ...
  static void Mutate(ASTNode& val, absl::BitGenRef prng,
    ...
    if (options.only_shrink) {
      if (is_current_value_mutable) {
        MutateCurrentValue(val, prng, options);
      }
      return;
    }
 ...
 private:
  static void MutateCurrentValue(ASTNode& val, absl::BitGenRef prng,
    ...
    ((ElementT::TypeId() == current_value.type_id
          ? (ElementT::Mutate(current_value, prng, options))
          : (void)0),
     ...);
  }
...
class InGrammarImpl
 ...
 public:
  ...
  void Mutate(ASTNode& val, absl::BitGenRef prng,
              const MutationOptions& options) {
    if (options.only_shrink && absl::Bernoulli(prng, 0.5) &&
        ShrinkByReplaceWithSubElementOfSameType(val, prng)) {
      return;
    }
    TopDomain::Mutate(val, prng, options);
  }
