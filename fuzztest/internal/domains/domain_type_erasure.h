...
class DomainModel final : public TypedDomainConcept<value_type_t<D>> {
 public:
  ...
  void UntypedMutate(GenericDomainCorpusType& val, absl::BitGenRef prng,
                     const MutationOptions& options) final {
    domain_.Mutate(val.GetAs<CorpusType>(), prng, options);
  }

  void UntypedUpdateMemoryDictionary(const GenericDomainCorpusType& val,
                                     ConstCmpTablesPtr cmp_tables) final {
  ...
  uint64_t UntypedMutateSelectedField(GenericDomainCorpusType& v,
    ...
    return domain_.MutateSelectedField(v.GetAs<CorpusType>(), prng, options,
