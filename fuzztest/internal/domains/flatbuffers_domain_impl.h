 ...
 private:
  ...
  auto GetDomainForField(const reflection::Field* absl_nonnull field) const {
    if constexpr (is_flatbuffers_array_tag_v<T>) {
      ...
      if constexpr (is_flatbuffers_enum_tag_v<ValueT>) {
        auto enum_object = schema_->enums()->Get(field->type()->index());
        return Domain<std::vector<typename ValueT::type>>{
            VectorOf(
                FlatbuffersEnumDomainImpl<typename ValueT::type>(enum_object))
                .WithSize(size)};
        ...
        return Domain<std::vector<const flatbuffers::Struct*>>(
            VectorOf(FlatbuffersStructUntypedDomainImpl{schema_, sub_object})
                .WithSize(size));
      } else if constexpr (std::is_integral_v<ValueT> ||
                           std::is_floating_point_v<ValueT>) {
        return Domain<std::vector<ValueT>>{
            VectorOf(Arbitrary<ValueT>()).WithSize(size)};
      } else {
