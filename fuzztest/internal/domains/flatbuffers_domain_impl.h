    ...
    Requires<FlatbuffersToolbox>(
        [](auto toolbox)
            -> decltype(toolbox.VerifySchemaBuffer(
             ...
             // Requires<FlatbuffersToolbox>(
             //     [](auto toolbox)
             //         -> decltype(toolbox.template offset_type<
//                     typename FlatbuffersToolbox::table_type>()) {});  // &&
// Requires<FlatbuffersToolbox>(
//     [](auto toolbox)
//         -> decltype(toolbox.GetTypeSize(
...
class FlatbuffersTableDomainWithBufferImpl
    : public domain_implementor::DomainBase<
          FlatbuffersTableDomainWithBufferImpl<T, FlatbuffersToolbox>,
          std::pair<const T*, const std::vector<uint8_t>*>,
          std::pair<corpus_type_t<
                        FlatbuffersTableUntypedDomainImpl<FlatbuffersToolbox>>,
                    std::vector<uint8_t>>> {
 public:
 ...
 private:
  FlatbuffersTableDomainImpl<T, FlatbuffersToolbox> inner_;
};
}  // namespace fuzztest::internal
