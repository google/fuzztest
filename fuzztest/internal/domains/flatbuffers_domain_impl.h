...
class FlatbuffersTableDomainImpl
    : public domain_implementor::DomainBase<
          ...
          FlatbuffersTableDomainImpl<Generated>,
          // ValueType - user facing type
          const Generated*,
          // CorpusType - internal representation of ValueType,
          // a pair with a map of field ids to field values and a serialized
          // buffer of the table.
          std::pair<
              absl::flat_hash_map<
                  // a.k.a. uint16_t
                  ...
                  // stored value and can copy it using exact type copy
                  // constructor via `CopyFrom` method.
 ...
 public:
  ...
  void Mutate(corpus_type& val, absl::BitGenRef prng,
    ...
    const FieldIdT selected_field_index =
  ...
  auto GetDomainForField(const reflection::Field* absl_nonnull field) const {
    if constexpr (std::is_same_v<T, FlatbuffersArrayTag>) {
      FUZZTEST_INTERNAL_CHECK(
          false, "Array in tables are not supported in flatbuffers.");
 ...
 private:
  ...
  bool IsTypeSupported(reflection::BaseType base_type) const {
    return flatbuffers::IsScalar(base_type) ||
  ...
  void UpdateCorpusBuffer(corpus_type& value) const {
    ...
    // Add all the fields to the builder and track their offsets.
    ...
    builder.Finish<flatbuffers::Table>(table_offset, nullptr);
  ...
  struct SerializeVisitor {
    const FlatbuffersTableDomainImpl& self;
    ...
    void Visit(const reflection::Field* absl_nonnull field) const {
      ...
      if constexpr (is_flatbuffers_enum_tag_v<T>) {
        ...
        if (field->optional() && !table->CheckField(field->offset())) {
          ...
          inner_value = std::make_optional(table->GetField<typename T::type>(
        ...
        if (field->optional() && !table->CheckField(field->offset())) {
          ...
          inner_value = std::make_optional(
              table->GetField<T>(field->offset(), field->default_integer()));
        }
        ...
        if (field->optional() && !table->CheckField(field->offset())) {
          ...
          inner_value = std::make_optional(
              table->GetField<T>(field->offset(), field->default_real()));
        }
        ...
        if (!table->CheckField(field->offset())) {
          ...
          inner_value = std::make_optional(
              table->GetPointer<flatbuffers::String*>(field->offset())->str());
        }
      }
  ...
  // Create out-of-line table fields, see `UpdateCorpusBuffer` for details.
  ...
  // offsets for "out-of-line fields". See `UpdateCorpusBuffer` for details.
  struct TableBuilderVisitor {
    const FlatbuffersTableDomainImpl& self;
    flatbuffers::FlatBufferBuilder& builder;
    const std::unordered_map<FieldIdT, flatbuffers::uoffset_t>& offsets;
    const typename corpus_type::first_type::value_type::second_type&
        corpus_value;
    ...
    void Visit(const reflection::Field* absl_nonnull field) const {
      ...
      if constexpr (std::is_integral_v<T> || std::is_floating_point_v<T> ||
        ...
        if (auto it = offsets.find(field->id()); it != offsets.end()) {
          builder.AddOffset(field->offset(), flatbuffers::Offset<>(it->second));
        }
      }
    }
  };
  ...
  struct InitializeVisitor {
    FlatbuffersTableDomainImpl& self;
    absl::BitGenRef prng;
    corpus_type& val;
