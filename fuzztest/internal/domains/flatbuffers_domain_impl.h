...
class FlatbuffersTableUntypedDomainImpl
 ...
 public:
  ...
  template <typename T>
  auto GetDefaultDomain(const reflection::Field* absl_nonnull field) const {
    if constexpr (std::is_same_v<T, FlatbuffersArrayTag>) {
      ...
      FUZZTEST_INTERNAL_CHECK(
          field->type()->base_type() >= reflection::BaseType::Byte &&
              field->type()->base_type() <= reflection::BaseType::ULong &&
              field->type()->index() >= 0,
          "Fields is not an enum.");
  ...
  auto& GetCachedDomain(const reflection::Field* field) const {
    using DomainT = decltype(GetDefaultDomain<T>(field));
  ...
  uint64_t CountNumberOfFields(corpus_type& val) {
    ...
    for (const auto* field : *table_object_->fields()) {
      ...
      // TODO: Add support for tables and vectors.
    }
    return field_count;
  }
    ...
    for (const auto& [id, field_corpus] : corpus_value) {
      ...
      if (field == nullptr) {
        return absl::InvalidArgumentError(
            absl::StrCat("Field id ", id, " is not found in the table."));
      }
      absl::Status result;
