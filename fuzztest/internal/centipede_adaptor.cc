...
void CentipedeTerminationHandler(int unused_signum) {
  Runtime::instance().SetTerminationRequested();
  RequestEarlyStop(EXIT_FAILURE);
}

void InstallCentipedeTerminationHandler() {
  [[maybe_unused]] bool install_once = [] {
    for (int signum : {SIGTERM, SIGINT, SIGHUP}) {
      struct sigaction new_sigact = {};
      sigemptyset(&new_sigact.sa_mask);
      new_sigact.sa_handler = CentipedeTerminationHandler;

      // We make use of the SA_ONSTACK flag so that signal handlers are executed
      // on a separate stack. This is needed to properly handle cases where
      // stack space is limited and the delivery of a signal needs to be
      // properly handled.
