...
bool CentipedeFuzzerAdaptor::Run(int* argc, char*** argv, RunMode mode,
  ...
  fuzzer_impl_.fixture_driver_->RunFuzzTest([&, this]() {
    ...
    if (runner_mode) {
      ...
      result = centipede::RunnerMain(argc != nullptr ? *argc : 1,
    ...
    if (fuzzer_impl_.ReplayInputsIfAvailable(configuration)) {
      result = 0;
      return;
    }
  ...
  [&] {
    ...
    result = centipede::CentipedeMain(env, factory);
    return;
  }();
  return result == 0;
}
