...
void Environment::UpdateWithTargetConfig(
  ...
  if (replay_coverage_attempts == Default().replay_coverage_attempts) {
    replay_coverage_attempts = config.replay_coverage_attempts;
  }
  if (batch_triage_suspect_only == Default().batch_triage_suspect_only) {
    batch_triage_suspect_only = config.batch_triage_suspect_only;
  }
