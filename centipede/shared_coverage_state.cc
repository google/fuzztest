...
SharedCoverageState shared_coverage_state
    __attribute__((used, init_priority(199)));
...
void PostProcessSharedCoverage(int target_return_value) {
...
__attribute__((noinline)) void CheckStackLimit(uintptr_t sp) {
  if (run_time_flags.centipede_runner_flags) {
    Initializer();
  }
  ...
  if (stack_limit > 0 && tls.stack_region_low &&
    ...
    if (shared_coverage_state.test_not_running) return;
