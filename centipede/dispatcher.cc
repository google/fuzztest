...
inline void DispatcherCheck(bool condition, const char* absl_nonnull error) {
  if (!condition) {
...
std::optional<std::string_view> GetDispatcherFlag(const char* flag_header) {
  ...
  const char* end = std::strstr(value_beg, ":");
  if (!end) return std::nullopt;
  return std::string_view{value_beg, static_cast<size_t>(end - value_beg)};
}

bool HasDispatcherSwitchFlag(const char* switch_flag) {
  const char* dispatcher_flags = GetDispatcherFlags();
  if (dispatcher_flags == nullptr) return false;
  // Extract "value" from ":flag=value:"
  return strstr(dispatcher_flags, switch_flag) != nullptr;
}
...
constexpr char kDispatcherOutputsBlobSequencePathFlagHeader[] =
    ":arg2=";  // TODO: better flag names.

BlobSequence* GetInputsBlobSequence() {
  static auto result = []() -> BlobSequence* {
    if (strstr(GetDispatcherFlags(), ":shmem:") == nullptr) {
      return nullptr;
    }
    auto input_path =
...
void TrySetFileContents(std::string_view path, const void* contents,
                        size_t size) {
  ...
  if (size > 0) {
    DispatcherCheck(contents != nullptr,
                    "contents cannot be nullopt when size > 0");
...
int DispatcherDoMutate(const FuzzTestDispatcherCallbacks* callbacks) {
  ...
  if (!MutationResult::WriteHasCustomMutator(has_mutate, *outputs_blobseq)) {
    fprintf(stderr, "Failed to write custom mutator indicator!\n");
    return EXIT_FAILURE;
  }
  if (!has_mutate) {
    fprintf(stderr, "No custom mutator!\n");
    return EXIT_SUCCESS;
  }
  ...
  if (!IsMutationRequest(inputs_blobseq->Read())) {
    fprintf(stderr, "Not mutation request!\n");
    return EXIT_FAILURE;
  }
  if (!IsNumMutants(inputs_blobseq->Read(), num_mutants)) {
    fprintf(stderr, "No num mutants\n");
    return EXIT_FAILURE;
  }
  if (!IsNumInputs(inputs_blobseq->Read(), num_inputs)) {
    fprintf(stderr, "No num inputs\n");
    return EXIT_FAILURE;
  }
  ...
  std::vector<FuzzTestDispatcherMutateInput> mutate_inputs;
  mutate_inputs.reserve(num_inputs);
  for (size_t i = 0; i < num_inputs; ++i) {
    ...
    mutate_inputs.push_back(FuzzTestDispatcherMutateInput{
...

  {
    ...
    callbacks->mutate(mutate_inputs.data(), mutate_inputs.size(), num_mutants,
                      /*shrink=*/0);
  }
  return EXIT_SUCCESS;
}
...
int DispatcherDoExecute(const FuzzTestDispatcherCallbacks* callbacks) {
  DispatcherCheck(callbacks->execute != nullptr,
                  "execute callback must be set");
  auto* inputs_blobseq = GetInputsBlobSequence();
  auto* outputs_blobseq = GetOutputsBlobSequence();
  DispatcherCheck(inputs_blobseq != nullptr && outputs_blobseq != nullptr,
...
using fuzztest::internal::kDispatcherFailureDescriptionPathFlagHeader;
using fuzztest::internal::kDispatcherFailureSignaturePathFlagHeader;
...
using fuzztest::internal::TrySetFileContents;
...
void FuzzTestDispatcherEmitTestName(const char* name) {
  ...
  FILE* f = fopen(test_output_path.c_str(), "w");
  if (f == nullptr) {
    perror("FAILURE: fopen()");
  }
  fclose(f);
...
void FuzzTestDispatcherEmitFeedbackAs1BitCounters(
  ...
  DispatcherCheck(output != nullptr, "outputs blob sequence must exist");
  DispatcherCheck(
      BatchResult::WriteOneFeatureVec(counter_list, num_counters, *output),
...
void FuzzTestDispatcherEmitExecutionMetadata(const void* metadata,
  ...
  DispatcherCheck(GetDispatcherAction() == DispatcherAction::kTestExecute &&
                      in_test_callback,
                  "must be called side test callback of executing");
  auto* output = GetOutputsBlobSequence();
  DispatcherCheck(output != nullptr, "outputs blob sequence must exist");
...
void FuzzTestDispatcherEmitFailure(FuzzTestDispatcherFailureType type,
                                   const char* description,
                                   const void* signature,
                                   size_t signature_size) {
  [[maybe_unused]] static bool write_once = [&] {
    if (const auto failure_description_path =
      ...
      switch (type) {
        case kFuzzTestInputFailure:
          DispatcherCheck(
              GetDispatcherAction() == DispatcherAction::kTestExecute &&
                  in_test_callback,
              "must be called side test callback of executing");
          final_description.append("INPUT FAILURE: ");
          break;
        case kFuzzTestIgnoredFailure:
          final_description.append("IGNORED FAILURE: ");
          break;
        case kFuzzTestSetupFailure:
          final_description.append("SETUP FAILURE: ");
          break;
        case kFuzzTestSkippedTest:
          final_description.append("SKIPPED TEST: ");
          break;
        default:
          DispatcherCheck(false, "unknown failure type to emit");
      }
      final_description.append(description);
      TrySetFileContents(*failure_description_path, final_description.data(),
                         final_description.size());
    }
    if (const auto failure_signature_path =
      ...
      TrySetFileContents(*failure_signature_path, signature, signature_size);
    }
    return true;
  }();
}
