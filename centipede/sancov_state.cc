...
void PostProcessSancov(bool reject_input) {
  ...
  sancov_state->processed_metadata.cmp_data.clear();

  if (sancov_state->flags.use_auto_dictionary) {
    sancov_state->ForEachTls([](ThreadLocalSancovState& tls) {
      AppendCmpEntries(tls.cmp_trace2, sancov_state->processed_metadata);
      AppendCmpEntries(tls.cmp_trace4, sancov_state->processed_metadata);
      AppendCmpEntries(tls.cmp_trace8, sancov_state->processed_metadata);
      AppendCmpEntries(tls.cmp_traceN, sancov_state->processed_metadata);
    });
  }
...
void SanCovRuntimePopulateExecutionMetadata(ExecutionMetadata& metadata) {
  metadata = fuzztest::internal::sancov_state->processed_metadata;
}
