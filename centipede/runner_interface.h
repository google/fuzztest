...
class RunnerCallbacks {
 public:
  ...
  virtual void GetPresetSeedInputs(std::function<void(void*)> seed_callback);
  // Generates a random seed input by calling `seed_callback` on it.
  // The default implementation generates a single-byte input {0}.
  virtual void GetRandomSeedInput(std::function<void(void*)> seed_callback);
  ...
  // Default is return Mutate(origin, origin_metadata)
  ...
  virtual void SerializeInput(void* input,
                              std::function<void(ByteSpan)> bytes_sink) = 0;
  virtual void* DeserializeInput(ByteSpan input_bytes) = 0;
