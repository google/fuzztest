...
[[noreturn]] static void *WatchdogThread(void *unused) {
  tls.ignore = true;
  state.watchdog_thread_ready = true;
  while (true) {
    sleep(1);
...
GlobalRunnerState::GlobalRunnerState() {
  ...
  CentipedeSetUpRunnerFlags(getenv("CENTIPEDE_RUNNER_FLAGS"));
  tls.OnThreadStart();
...
extern "C" void CentipedeSetUpRunnerFlags(const char *runner_flag_string) {
  centipede::state.SetUpCentipedeRunnerFlags(runner_flag_string);
  centipede::state.MaybeStartWatchdogThread();
  centipede::SetLimits();
}
