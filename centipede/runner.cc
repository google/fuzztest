      ...
      while (true) {
        ...
        if (first) {
          ...
          fprintf(stderr,
                  ...
                  "argv[0]: %s flags: %s\n",
                  req == kPersistentModeRequestExit ? "exiting persistent mode"
                                                    : "persistent mode batch",
                  argv[0], state.centipede_runner_flags);
        }
        if (req == kPersistentModeRequestExit) break;
        RunnerCheck(req == kPersistentModeRequestRunOnce,
                    "Unknown persistent mode request");
        const int result = HandleSharedMemoryRequest(callbacks, inputs_blobseq,
                                                     outputs_blobseq);
        inputs_blobseq.Reset();
        outputs_blobseq.Reset();
        if (write(state.persistent_mode_socket, &result, sizeof(result)) !=
            sizeof(result)) {
          fprintf(stderr,
