...
void GlobalRunnerState::StartWatchdogThread() {
  fprintf(stderr,
          ...
          sancov_state.flags.stack_limit_kb.load());
...
PostProcessCoverage(int target_return_value) {
  PostProcessSancov(target_return_value);
...
static void SetLimits() {
  ...
  if (vm_size_in_bytes < one_tb) {
    size_t address_space_limit_mb =
        flag_helper.HasIntFlag(":address_space_limit_mb=", 0);
...
static void MaybePopulateReversePcTable() {
  const char *pcs_file_path = flag_helper.GetStringFlag(":pcs_file_path=");
...
GlobalRunnerState::GlobalRunnerState() {
  ...
  // Compute main_object.
  sancov_state.main_object =
      GetDlInfo(flag_helper.GetStringFlag(":dl_path_suffix="));
  ...
  if (flag_helper.HasFlag(":dump_binary_info:")) {
    RunnerCheck(flag_helper.arg1 && flag_helper.arg2 && flag_helper.arg3,
                "dump_binary_info requires 3 arguments");
    if (!flag_helper.arg1 || !flag_helper.arg2 || !flag_helper.arg3)
      _exit(EXIT_FAILURE);
    DumpPcTable(flag_helper.arg1);
    DumpCfTable(flag_helper.arg2);
    DumpDsoTable(flag_helper.arg3);
    _exit(EXIT_SUCCESS);
  }
...
[[maybe_unused]] auto fake_reference_for_runner = &RunnerSancov;
...
GlobalRunnerState::~GlobalRunnerState() {
  ...
  if (!state.centipede_runner_main_executed && flag_helper.HasFlag(":shmem:")) {
    ...
    SharedMemoryBlobSequence outputs_blobseq(flag_helper.arg2);
...
int RunnerMain(int argc, char **argv, RunnerCallbacks &callbacks) {
  ...
  fprintf(stderr, "Centipede fuzz target runner; argv[0]: %s flags: %s\n",
          argv[0], flag_helper.centipede_runner_flags);

  if (flag_helper.HasFlag(":dump_configuration:")) {
    DumpSerializedTargetConfigToFile(callbacks,
                                     /*output_file_path=*/flag_helper.arg1);
    return EXIT_SUCCESS;
  }

  if (flag_helper.HasFlag(":dump_seed_inputs:")) {
    // Seed request.
    DumpSeedsToDir(callbacks, /*output_dir=*/flag_helper.arg1);
    return EXIT_SUCCESS;
  }

  // Inputs / outputs from shmem.
  if (flag_helper.HasFlag(":shmem:")) {
    if (!flag_helper.arg1 || !flag_helper.arg2) return EXIT_FAILURE;
    SharedMemoryBlobSequence inputs_blobseq(flag_helper.arg1);
    SharedMemoryBlobSequence outputs_blobseq(flag_helper.arg2);
...
extern "C" void CentipedeSetStackLimit(size_t stack_limit_kb) {
  fprintf(stderr, "CentipedeSetStackLimit: changing stack_limit_kb to %zu\n",
          stack_limit_kb);
  fuzztest::internal::sancov_state.flags.stack_limit_kb = stack_limit_kb;
}
