...
GlobalRunnerState::GlobalRunnerState() {
  ...
  if (const char *runner_flags_env = getenv("CENTIPEDE_RUNNER_FLAGS")) {
    RunnerCheck(CentipedeSetUpRunnerFlags(strdup(runner_flags_env)),
                "Failed to set up the runner flags from environment");
  } else {
    // Populate the default state.
    UpdateStateFromCentipedeRunnerFlags();
  }
  tls.OnThreadStart();
...
int RunnerMain(int argc, char **argv, RunnerCallbacks &callbacks) {
  ...
  fprintf(stderr, "Centipede fuzz target runner; argv[0]: %s flags: %s\n",
          argv[0], state.centipede_runner_flags.load());
...
static std::atomic_flag centipede_runner_flags_is_set;

extern "C" bool CentipedeSetUpRunnerFlags(const char *runner_flag_string) {
  if (centipede_runner_flags_is_set.test_and_set()) return false;
  centipede::state.centipede_runner_flags = runner_flag_string;
  centipede::state.UpdateStateFromCentipedeRunnerFlags();
  centipede::state.StartWatchdogThread();
  centipede::SetLimits();
  return true;
}
