...
static void CheckForceAbortTimeout() {
  ...
  if (time(nullptr) > state.force_abort_deadline) {
    ...
    struct sigaction sa = {};
    // Use an alternate stack since it is possible that the regular stack is
    // full, corrupted, or otherwise unusable during crashes.
    sa.sa_flags = SA_ONSTACK;
    sigemptyset(&sa.sa_mask);
    // Overwrite with kernel default to prevent hangs within Centipede's crash
    // handler.
    sa.sa_handler = SIG_DFL;
    sigaction(SIGABRT, &sa, nullptr);
    // This unblocks and raises `SIGABRT`.
    std::abort();
  }
}
