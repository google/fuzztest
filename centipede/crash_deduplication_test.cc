...
TEST_F(OrganizeInputsTest, RenamesOldStyleCrashFileToNewStyle) {
  ...
  OrganizeOldInputsAndStoreNewCrashes(
      regression_dir(), crashing_dir(), env(), factory,
...
TEST_F(OrganizeInputsTest, KeepsReproducibleCrashesWithSameCrashSignature) {
...
TEST_F(OrganizeInputsTest, KeepsFlakyCrashAndUpdatesModificationTime) {
...
TEST_F(OrganizeInputsTest, KeepsIrreproducibleCrashAndCopiesToRegressionDir) {
  SetContentsAndGetPath(crashing_dir(), "bug-csig-isig", "input");
  FakeCentipedeCallbacks callbacks(env(), /*crashing_inputs=*/{});
  NonOwningCallbacksFactory factory(callbacks);
...
TEST_F(OrganizeInputsTest, KeepsSetupFailureCrashAndCopiesToRegressionDir) {
  SetContentsAndGetPath(crashing_dir(), "bug-csig-isig", "input");
...
TEST_F(OrganizeInputsTest, DoesNotProcessInputsInRegressionDir) {
...
TEST_F(OrganizeInputsTest, ReplacesIrreproducibleCrashAtFileLimit) {
  SetContentsAndGetPath(crashing_dir(), "bug1-csig1-isig1", "repro1");
  SetContentsAndGetPath(crashing_dir(), "bug2-csig2-isig2", "repro2");
  SetContentsAndGetPath(crashing_dir(), "bug3-csig3-isig3", "repro3");
  SetContentsAndGetPath(crashing_dir(), "bug4-csig4-isig4", "repro4");
