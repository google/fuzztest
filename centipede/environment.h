...
struct Environment {
  ...
  //----------------------------------------------------------------------------
  // Centipede properties. Initialized in CTOR from the flags defined in
  // environment.cc.
  //----------------------------------------------------------------------------
  ...
  // NOTE: Set once in CTOR - don't override.
  ...
  size_t NumBatches() const {
    return num_runs / batch_size + (num_runs % batch_size != 0 ? 1 : 0);
  }

  size_t BatchSize(size_t num_done_runs) const {
    return std::min(batch_size, num_runs - num_done_runs);
  }
  ...
  //----------------------------------------------------------------------------
  // Whether certain actions should be performed under given circumstances.
  //----------------------------------------------------------------------------
