...
struct MinimizerState {
 public:
  ...
  MinimizerState(size_t capacity, ByteArray crasher)
      : capacity_(capacity),
        largest_crasher_index_(0),
        crashers_{std::move(crasher)} {}
  ...
  void AddCrasher(ByteArray new_crasher, CrashDetails details) {
    absl::MutexLock lock(&mutex_);
    if (crashers_[largest_crasher_index_].size() < new_crasher.size()) {
      return;
    }
    ...
    if (crashers_.size() < capacity_) {
      crashers_.push_back(std::move(new_crasher));
    } else {
      std::swap(crashers_[largest_crasher_index_], new_crasher);
    }
    for (size_t i = 0; i < crashers_.size(); ++i) {
      if (crashers_[largest_crasher_index_].size() < crashers_[i].size()) {
        largest_crasher_index_ = i;
      }
    }
  }
 ...
 private:
  ...
  size_t largest_crasher_index_ ABSL_GUARDED_BY(mutex_);
  ;
...
static void MinimizeCrash(const Environment& env,
  ...
  for (size_t i = 0; i < num_batches; ++i) {
    ...
    constexpr int kNumAttemptsToGenerateSmallerMutants = 2;
    std::vector<ByteArray> smaller_mutants;
    for (int i = 0; i < kNumAttemptsToGenerateSmallerMutants; ++i) {
      ...
      if (!smaller_mutants.empty()) break;
    }

    if (smaller_mutants.empty()) continue;
