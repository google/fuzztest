...
void PopulateMetadata(const fuzztest::internal::ExecutionMetadata* metadata,
                      TablesOfRecentCompares& cmp_tables) {
  ...
  metadata->ForEachCmpEntry([&cmp_tables](fuzztest::internal::ByteSpan a,
    ...
    if (size < FuzzTestMutator::kMinCmpEntrySize) return;
    if (size > FuzzTestMutator::kMaxCmpEntrySize) return;
    if (size == 2) {
...
std::vector<ByteArray> FuzzTestMutator::MutateMany(
  ...
  for (int i = 0; i < num_mutants; ++i) {
    ...
    if (!cmp_tables[index].has_value() && inputs[index].metadata != nullptr) {
      ...
      PopulateMetadata(inputs[index].metadata, *cmp_tables[index]);
    }
