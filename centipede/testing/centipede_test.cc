...
TEST(Centipede, MutateViaExternalBinaryWithCmpData) {
  ...
  const std::string binary_with_copying_custom_mutator =
      GetDataDependencyFilepath("centipede/testing/cmp_data_fuzz_target");
  ...
  mutants.resize(100);
  EXPECT_TRUE(callbacks.MutateViaExternalBinary(
      binary_with_copying_custom_mutator, /*cmp_data=*/{1, 2, 3, 4, 5},
      /*inputs=*/{{0}}, mutants));
  EXPECT_THAT(mutants, testing::AllOf(
                           testing::SizeIs(100),
                           testing::Each(testing::ElementsAre(1, 2, 3, 4, 5))));
  mutants.resize(100);
  EXPECT_TRUE(callbacks.MutateViaExternalBinary(
      binary_with_agnostic_custom_mutator, /*cmp_data=*/{1, 2, 3, 4, 5},
      /*inputs=*/{{0}}, mutants));
  EXPECT_THAT(
      mutants,
      testing::AllOf(
          testing::SizeIs(100),
          testing::Each(testing::Not(testing::ElementsAre(1, 2, 3, 4, 5)))));
}
