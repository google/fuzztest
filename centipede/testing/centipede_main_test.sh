...
test_timeouts() {
  ...
  centipede::ensure_empty_dir "${WD}"
  centipede::ensure_empty_dir "${TMPCORPUS}"
  echo -n "slo" >"${TMPCORPUS}"/a

  echo "============ ${FUNC}: fuzz with --timeout_per_input"
  test_fuzz --workdir="${WD}" --corpus_dir="${TMPCORPUS}" --num_runs=0 --timeout_per_input=2 | tee "${LOG}"
  ...
  echo "============ ${FUNC}: fuzz with --timeout_per_input --no_report_timeouts"
  test_fuzz --workdir="${WD}" --corpus_dir="${TMPCORPUS}" --num_runs=0 --timeout_per_input=2 --no_report_timeouts | tee "${LOG}"
