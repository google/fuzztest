...
{
    CENTIPEDE_RUNNER_FLAGS=":no_report_timeouts:timeout_per_input=2:" "${target}" "${slo}";
    echo "$?" > "${TEST_TMPDIR}/no_report_timeouts_exit_code";
} 2>&1 | grep "Per-input timeout exceeded" | grep "exiting without reporting as an error"
((`cat "${TEST_TMPDIR}/no_report_timeouts_exit_code"` == 0))
