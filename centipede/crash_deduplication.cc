...
absl::flat_hash_map<std::string, CrashDetails> GetCrashesFromWorkdir(
  ...
  for (size_t shard_idx = 0; shard_idx < total_shards; ++shard_idx) {
    ...
    for (std::string& crashing_input_file : crashing_input_files) {
      ...
      crashes.insert({std::move(crash_signature),
                      CrashDetails{std::move(crashing_input_file_name),
                                   std::move(crash_description),
                                   std::move(crashing_input_file)}});
    }
  }
  return crashes;
}
...
void OrganizeOldInputsAndStoreNewCrashes(
  ...
  old_input_files.reserve(old_input_files.size() +
                          regression_input_files.size());
  ...
  for (std::string& old_input_file : old_input_files) {
    ...
    if (!(is_reproducible && batch_result.IsOrdinaryFailure())) {
    ...
    std::string new_input_file_name =
        absl::StrCat(input_file_components.bug_id, "-",
                     input_file_components.crash_signature, "-",
  ...
  for (const std::string& old_input_file : irreproducible_input_files) {
    ...
    if (!input_file_components.ok()) {
      ...
      FUZZTEST_LOG_IF(ERROR, !status.ok())
          << "Failed to delete file " << old_input_file
          << ". Status: " << status;
      continue;
    }

    decltype(reproduced_crashes)::const_iterator crash_it =
        reproduced_crashes.find(input_file_components->crash_signature);
    bool is_new_crash = false;
    if (crash_it == reproduced_crashes.end()) {
      crash_it = new_crashes.find(input_file_components->crash_signature);
      if (crash_it != new_crashes.end()) {
        crash_it = reproduced_crashes.insert(*crash_it).first;
        is_new_crash = true;
      } else {
        crash_it = reproduced_crashes.end();
      }
    }
    if (crash_it != reproduced_crashes.end()) {
      // Reproducible: Found another input that produces the same crash
      // signature.
      ...
      if (new_input_file == old_input_file) {
        ...
        const auto status =
            RemoteFileCopy(crash_it->second.file_path, new_input_file);
        if (status.ok()) {
          crash_summary.AddCrash({/*id=*/new_input_file_name,
                                  /*category=*/crash_it->second.description,
                                  input_file_components->crash_signature,
          ...
          FUZZTEST_LOG(ERROR)
              << "Failed to copy file " << crash_it->second.file_path << " to "
              << new_input_file << ". Status: " << status;
          if (is_new_crash) {
  ...
  for (auto& [crash_signature, details] : new_crashes) {
    ...
    const std::string new_input_file_name = absl::StrCat(
        bug_id, "-", crash_signature, "-", details.input_signature);
    const std::string new_input_file =
        std::filesystem::path(crashing_dir) / new_input_file_name;
    const auto status = RemoteFileCopy(details.file_path, new_input_file);
    if (!status.ok()) {
      FUZZTEST_LOG(ERROR) << "Failed to copy file " << details.file_path
