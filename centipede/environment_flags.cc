...
#define CENTIPEDE_FLAG_ALIAS(TYPE, ALIAS_NAME, FLAG_NAME, DEFAULT)            \
  ABSL_FLAG(TYPE, ALIAS_NAME, DEFAULT, "Alias of --" #FLAG_NAME)              \
      .OnUpdate([]() {                                                        \
        absl::SetFlag(&FLAGS_##FLAG_NAME, absl::GetFlag(FLAGS_##ALIAS_NAME)); \
      });
#define CENTIPEDE_FUZZTEST_FLAG(TYPE, NAME, DEFAULT, DESC) \
  ABSL_FLAG(TYPE, NAME, DEFAULT, DESC);
...
#define CENTIPEDE_FLAG_ALIAS(TYPE, ALIAS_NAME, FLAG_NAME, DEFAULT)
#define CENTIPEDE_FUZZTEST_FLAG(_TYPE, NAME, _DEFAULT, _DESC) \
  absl::GetFlag(FLAGS_##NAME),
#include "./centipede/centipede_flags.inc"
#undef CENTIPEDE_FLAG
#undef CENTIPEDE_FLAG_ALIAS
