...
bool BatchResult::Read(BlobSequence &blobseq) {
  ...
  while (true) {
    ...
    if (blob.tag == kTagDispatcher32BitFeatures) {
      if (current_execution_result == nullptr) return false;
      const size_t u32_features_size = blob.size / sizeof(uint32_t);
      std::vector<uint32_t> copied_u32_features;
      copied_u32_features.resize(u32_features_size);
      std::memcpy(copied_u32_features.data(), blob.data,
                  u32_features_size * sizeof(uint32_t));
      FeatureVec &features = current_execution_result->mutable_features();
      for (uint32_t u32_feature : copied_u32_features) {
        features.push_back((u32_feature & 0x7fffffff) +
