...
class CentipedeCallbacks::PersistentModeServer {
 public:
  explicit PersistentModeServer(std::string server_path)
    ...
    PCHECK(listen(server_socket_, backlog) != -1);
  }
  ...
  bool RunBatch(absl::Time deadline, int &exit_code) {
    ...
    if (!WriteFd(conn_socket_, deadline, PersistentModeRequest::kRunBatch)) {
      LOG(ERROR) << "Failed to request the persistent mode client to run a "
                    "batch. Disconnecting..";
      Disconnect();
      return false;
    }
    if (!ReadFd(conn_socket_, deadline, exit_code)) {
      LOG(ERROR) << "Failed to receive the batch response from the persistent "
  ...
  void RequestExit(absl::Time deadline) {
    ...
    if (!WriteFd(conn_socket_, deadline, PersistentModeRequest::kExit)) {
      LOG(ERROR) << "Failed to request the persistent mode client to exit. "
 ...
 private:
  ...
  static bool PollFd(int fd, int event, absl::Time deadline) {
    CHECK(event == POLLIN || event == POLLOUT)
    ...
    if (poll_ret == 1 && (poll_fd.revents & (event | POLLHUP)) == event) {
      ...
      PLOG(ERROR) << "Persistent mode: poll() on connection failed";
    } else if (poll_ret == 0) {
  ...
  template <typename T>
  static bool ReadFd(int fd, absl::Time deadline, T &data) {
    static_assert(
    ...
    while (cursor < end) {
      ...
      if (r > 0) {
        cursor += r;
        CHECK(cursor <= end) << "read() returns more than needed on fd " << fd;
        continue;
      }
      if (r == 0) {
        LOG(ERROR) << "read() returned 0 early on fd " << fd;
        return false;
      }
      if (errno == EAGAIN || errno == EWOULDBLOCK || errno == EINTR) continue;
      PLOG(ERROR) << "read() fails on fd " << fd;
      return false;
    }
    return true;
  }
  ...
  template <typename T>
  static bool WriteFd(int fd, absl::Time deadline, const T &data) {
    static_assert(
        std::is_trivial_v<T> && std::is_standard_layout_v<T>,
    ...
    while (cursor < end) {
      ...
      if (r > 0) {
        cursor += r;
        CHECK(cursor <= end) << "write() returns more than needed on fd " << fd;
        continue;
      }
      if (r == 0) {
        LOG(ERROR) << "write() returned 0 early on fd " << fd;
        return false;
      }
      if (errno == EAGAIN || errno == EWOULDBLOCK || errno == EINTR) continue;
      PLOG(ERROR) << "write() fails on fd " << fd;
      return false;
    }
    return true;
  }
  ...
  bool EnsureConnection(absl::Time deadline) {
    ...
    do {
      if (!PollFd(server_socket_, POLLIN, deadline)) {
        LOG(ERROR) << "Persistent mode: polling failed on the server socket "
                   << server_path_;
        return false;
      }
      ...
      std::remove_if(
          ...
          [&](auto &extended_command) {
            if (extended_command->cmd.is_executing() &&
                extended_command->persistent_mode_server != nullptr) {
              const absl::Time deadline = absl::Now() + kCommandCleanupTimeout;
              extended_command->persistent_mode_server->RequestExit(deadline);
              const auto ret = extended_command->cmd.Wait(deadline);
              LOG_IF(ERROR, !ret.has_value())
                  << "Timed out while waiting for Command "
...
int CentipedeCallbacks::RunBatchForBinary(std::string_view binary) {
  ...
  const bool should_clean_up = [&] {
    ...
    if (extended_command.persistent_mode_server != nullptr &&
        extended_command.persistent_mode_server->RunBatch(deadline,
                                                          exit_code)) {
      return false;
    }
