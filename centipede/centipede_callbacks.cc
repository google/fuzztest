...
bool CentipedeCallbacks::MutateViaExternalBinary(
    std::string_view binary, const ByteArray &cmp_data,
    const std::vector<ByteArray> &inputs, std::vector<ByteArray> &mutants) {
  auto start_time = absl::Now();
  inputs_blobseq_.Reset();
  outputs_blobseq_.Reset();

  size_t num_inputs_written = execution_request::RequestMutation(
      mutants.size(), cmp_data, inputs, inputs_blobseq_);
