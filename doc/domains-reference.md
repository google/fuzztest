...
Random Bit Generator (URBG), pass the generator using `absl::BitGenRef` and
utilize the `Arbitrary<absl::BitGenRef>()` domain. This approach provides a
...
The `Arbitrary<absl::BitGenRef>()` domain also provides mocks for Abseil Random
Library distribution function (e.g., `absl::Uniform()`, `absl::Bernoulli()`).
These mocks directly interact with the underlying random sequence and aid the
discovery of boundary conditions and edge cases.
