...
TEST(FlatbuffersEnumDomainImplTest, ExcludedValuesAreNotGenerated) {
  ...
  internal::FlatbuffersEnumDomainImpl<uint8_t> domain(enum_def);
  domain.SetExcludedValues({internal::ByteEnum_Second});
  EXPECT_THAT(
...
TEST(FlatbuffersEnumDomainImplTest, InvalidEnumValuesAreRejected) {
  ...
  internal::FlatbuffersEnumDomainImpl<uint8_t> domain(enum_def);
  {
    typename internal::FlatbuffersEnumDomainImpl<
        uint8_t>::corpus_type invalid_value =
