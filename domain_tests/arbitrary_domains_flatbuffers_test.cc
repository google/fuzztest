...
TEST(FlatbuffersTableDomainImplTest, CountNumberOfFieldsWithNull) {
  flatbuffers::FlatBufferBuilder fbb;
  auto table_offset = internal::CreateNestedTestFbsTable(fbb);
  fbb.Finish(table_offset);
  auto table = flatbuffers::GetRoot<NestedTestFbsTable>(fbb.GetBufferPointer());
  ...
  auto corpus = domain.FromValue(table);
  ASSERT_TRUE(corpus.has_value());
  ASSERT_EQ(domain.CountNumberOfFields(corpus.value()), 1);
}
...
TEST(FlatbuffersTableDomainImplTest, VectorsCountNumberOfFieldsWithNull) {
  auto domain = Arbitrary<VectorsTestFbsTable>();
  {
    flatbuffers::FlatBufferBuilder fbb;
    auto table_offset = internal::CreateVectorsTestFbsTableDirect(fbb);
    fbb.Finish(table_offset);
    auto table =
        flatbuffers::GetRoot<VectorsTestFbsTable>(fbb.GetBufferPointer());
    ...
    auto corpus = domain.FromValue(table);
    ASSERT_TRUE(corpus.has_value());
    EXPECT_EQ(domain.CountNumberOfFields(corpus.value()), 14);
  }
  {
    flatbuffers::FlatBufferBuilder fbb;
    std::vector<flatbuffers::Offset<SimpleTestFbsTable>> table_offsets;
    ...
    auto vector_offset = fbb.CreateVector(table_offsets);
    internal::VectorsTestFbsTableBuilder table_builder(fbb);
    table_builder.add_t(vector_offset);
    auto table_offset = table_builder.Finish();
    fbb.Finish(table_offset);
    auto table =
        flatbuffers::GetRoot<VectorsTestFbsTable>(fbb.GetBufferPointer());
    ...
    auto corpus = domain.FromValue(table);
    ASSERT_TRUE(corpus.has_value());
    EXPECT_EQ(domain.CountNumberOfFields(corpus.value()), 26);
  }
}
...
TEST(FlatbuffersTableDomainImplTest, UnionCountNumberOfFieldsWithNull) {
  flatbuffers::FlatBufferBuilder fbb;
  auto table_offset = internal::CreateUnionTestFbsTable(fbb);
  fbb.Finish(table_offset);
  auto table = flatbuffers::GetRoot<UnionTestFbsTable>(fbb.GetBufferPointer());
  auto domain = Arbitrary<UnionTestFbsTable>();
  auto corpus = domain.FromValue(table);
  ASSERT_TRUE(corpus.has_value());
  ASSERT_EQ(domain.CountNumberOfFields(corpus.value()), 2);
}
...
TEST(FlatbuffersTableDomainImplTest, StructFieldsSerializeAndDeserialize) {
  flatbuffers::FlatBufferBuilder fbb;
  auto s = SimpleFbsStruct{true, 1.0, NestedFbsStruct{true},
                           internal::TestFbsEnum_Second};
  std::vector<SimpleFbsStruct> structs;
  structs.emplace_back(true, 1.0, NestedFbsStruct{true},
                       internal::TestFbsEnum_Second);
  structs.emplace_back(true, 2.0, NestedFbsStruct{true},
                       internal::TestFbsEnum_Third);
  auto s_vec = fbb.CreateVectorOfStructs(structs);
  auto s_offset = fbb.CreateStruct(s);
  std::vector<uint8_t> u_type_vec;
  u_type_vec.push_back(internal::StructUnion_SimpleFbsStruct);
  auto u_vec_type_offset = fbb.CreateVector(u_type_vec);
  auto u_vec_offset = fbb.CreateVector({s_offset.Union()});
  auto table_offset = internal::CreateStructTestFbsTable(
      fbb, &s, &s, s_vec, internal::StructUnion_SimpleFbsStruct,
      s_offset.Union(), u_vec_type_offset, u_vec_offset);
  fbb.Finish(table_offset);
  auto table = flatbuffers::GetRoot<StructTestFbsTable>(fbb.GetBufferPointer());

  auto domain = Arbitrary<StructTestFbsTable>();
  auto corpus = domain.FromValue(table);
  auto ir = domain.SerializeCorpus(corpus.value());
  auto new_corpus = domain.ParseCorpus(ir);
  ASSERT_TRUE(new_corpus.has_value());
  ...
  auto new_table = domain.GetValue(*new_corpus);
  ASSERT_NE(new_table, nullptr);
  ASSERT_NE(new_table->s(), nullptr);
  EXPECT_EQ(new_table->s()->b(), true);
  EXPECT_EQ(new_table->s()->f(), 1.0);
  EXPECT_EQ(new_table->s()->s().b(), true);
  EXPECT_EQ(new_table->s()->e(), internal::TestFbsEnum_Second);
  ASSERT_NE(new_table->s_req(), nullptr);
  EXPECT_EQ(new_table->s_req()->b(), true);
  EXPECT_EQ(new_table->s_req()->f(), 1.0);
  EXPECT_EQ(new_table->s_req()->s().b(), true);
  EXPECT_EQ(new_table->s_req()->e(), internal::TestFbsEnum_Second);
  ASSERT_NE(new_table->s_vec(), nullptr);
  ASSERT_EQ(new_table->s_vec()->size(), 2);
  auto s_vec_zero = new_table->s_vec()->Get(0);
  ASSERT_NE(s_vec_zero, nullptr);
  EXPECT_EQ(s_vec_zero->b(), true);
  EXPECT_EQ(s_vec_zero->f(), 1.0);
  EXPECT_EQ(s_vec_zero->s().b(), true);
  EXPECT_EQ(s_vec_zero->e(), internal::TestFbsEnum_Second);
  auto s_vec_one = new_table->s_vec()->Get(1);
  ASSERT_NE(s_vec_one, nullptr);
  EXPECT_EQ(s_vec_one->b(), true);
  EXPECT_EQ(s_vec_one->f(), 2.0);
  EXPECT_EQ(s_vec_one->s().b(), true);
  EXPECT_EQ(s_vec_one->e(), internal::TestFbsEnum_Third);
  EXPECT_EQ(new_table->u_type(), internal::StructUnion_SimpleFbsStruct);
  ASSERT_NE(new_table->u(), nullptr);
  EXPECT_EQ(new_table->u_as_SimpleFbsStruct()->b(), true);
  EXPECT_EQ(new_table->u_as_SimpleFbsStruct()->f(), 1.0);
  EXPECT_EQ(new_table->u_as_SimpleFbsStruct()->s().b(), true);
  EXPECT_EQ(new_table->u_as_SimpleFbsStruct()->e(),
            internal::TestFbsEnum_Second);
  EXPECT_EQ(new_table->u_vec_type()->size(), 1);
  EXPECT_EQ(new_table->u_vec_type()->Get(0),
            internal::StructUnion_SimpleFbsStruct);
  auto u_vec = new_table->u_vec();
  ASSERT_NE(u_vec, nullptr);
  ASSERT_EQ(u_vec->size(), 1);
  auto u_vec_zero = u_vec->Get(0);
  ASSERT_NE(u_vec_zero, nullptr);
  EXPECT_EQ(static_cast<const SimpleFbsStruct*>(u_vec_zero)->b(), true);
  EXPECT_EQ(static_cast<const SimpleFbsStruct*>(u_vec_zero)->f(), 1.0);
  EXPECT_EQ(static_cast<const SimpleFbsStruct*>(u_vec_zero)->s().b(), true);
  EXPECT_EQ(static_cast<const SimpleFbsStruct*>(u_vec_zero)->e(),
            internal::TestFbsEnum_Second);
}

TEST(FlatbuffersTableDomainImplTest, StructCountNumberOfFieldsWithNull) {
  auto s = SimpleFbsStruct{true, 1.0, NestedFbsStruct{true},
                           internal::TestFbsEnum_Second};
  flatbuffers::FlatBufferBuilder fbb;
  auto table_offset = internal::CreateStructTestFbsTable(fbb, nullptr, &s);
  fbb.Finish(table_offset);
  auto table = flatbuffers::GetRoot<StructTestFbsTable>(fbb.GetBufferPointer());
  auto domain = Arbitrary<StructTestFbsTable>();
  auto corpus = domain.FromValue(table);
  ASSERT_TRUE(corpus.has_value());
  ASSERT_EQ(domain.CountNumberOfFields(corpus.value()), 9);
}

TEST(FlatbuffersTableDomainImplTest, StructFieldsEventuallyMutate) {
  auto domain = Arbitrary<StructTestFbsTable>();
  ...
  verify_field_changes("s.b", [](auto v) {
    return v && v->s() ? std::make_optional(v->s()->b()) : std::nullopt;
  });
  verify_field_changes("s.f", [](auto v) {
    return v && v->s() ? std::make_optional(v->s()->f()) : std::nullopt;
  });
  verify_field_changes("s.s.b", [](auto v) {
    return v && v->s() ? std::make_optional(v->s()->s().b()) : std::nullopt;
  });
  verify_field_changes("s.e", [](auto v) {
    return v && v->s() ? std::make_optional(v->s()->e()) : std::nullopt;
  });
  verify_field_changes("s_req.b", [](auto v) {
    return v && v->s_req() ? std::make_optional(v->s_req()->b()) : std::nullopt;
  });
  verify_field_changes("s_req.f", [](auto v) {
    return v && v->s_req() ? std::make_optional(v->s_req()->f()) : std::nullopt;
  });
  verify_field_changes("s_req.s.b", [](auto v) {
    return v && v->s_req() ? std::make_optional(v->s_req()->s().b())
                           : std::nullopt;
  });
  verify_field_changes("s_req.e", [](auto v) {
    return v && v->s_req() ? std::make_optional(v->s_req()->e()) : std::nullopt;
  });
  verify_field_changes("s_vec[0].b", [](auto v) {
    return v && v->s_vec() && v->s_vec()->size() > 0
               ? std::make_optional(v->s_vec()->Get(0)->b())
               : std::nullopt;
  });
  verify_field_changes("s_vec[0].f", [](auto v) {
    return v && v->s_vec() && v->s_vec()->size() > 0
               ? std::make_optional(v->s_vec()->Get(0)->f())
               : std::nullopt;
  });
  verify_field_changes("s_vec[0].s.b", [](auto v) {
    return v && v->s_vec() && v->s_vec()->size() > 0
               ? std::make_optional(v->s_vec()->Get(0)->s().b())
               : std::nullopt;
  });
  verify_field_changes("s_vec[0].e", [](auto v) {
    return v && v->s_vec() && v->s_vec()->size() > 0
               ? std::make_optional(v->s_vec()->Get(0)->e())
               : std::nullopt;
  });
  verify_field_changes("u_type", [](auto v) {
    return v ? std::make_optional(v->u_type()) : std::nullopt;
  });
  verify_field_changes("u_as_SimpleFbsStruct", [](auto v) {
    return v && v->u_type() == internal::StructUnion_SimpleFbsStruct
               ? std::make_optional(v->u_as_SimpleFbsStruct())
               : std::nullopt;
  });
  verify_field_changes("u_as_NestedFbsStruct", [](auto v) {
    return v && v->u_type() == internal::StructUnion_NestedFbsStruct
               ? std::make_optional(v->u_as_NestedFbsStruct())
               : std::nullopt;
  });
  verify_field_changes("u_vec_type", [](auto v) {
    return v ? std::make_optional(v->u_type()) : std::nullopt;
  });
  verify_field_changes("u_vec[0].as_SimpleFbsStruct", [](auto v) {
    return v && v->u_vec_type()->size() > 0 &&
                   v->u_vec_type()->Get(0) ==
                       internal::StructUnion_SimpleFbsStruct &&
                   v->u_vec() && v->u_vec()->size() > 0
               ? std::make_optional(
                     static_cast<const SimpleFbsStruct*>(v->u_vec()->Get(0)))
               : std::nullopt;
  });
  verify_field_changes("u_vec[0].as_NestedFbsStruct", [](auto v) {
    return v && v->u_vec_type()->size() > 0 &&
                   v->u_vec_type()->Get(0) ==
                       internal::StructUnion_NestedFbsStruct &&
                   v->u_vec() && v->u_vec()->size() > 0
               ? std::make_optional(
                     static_cast<const NestedFbsStruct*>(v->u_vec()->Get(0)))
               : std::nullopt;
  });
}
}  // namespace
