...
inline bool Eq<BoolStruct>(const BoolStruct& lhs, const BoolStruct& rhs) {
  return Eq(lhs.b(), rhs.b()) && ArrayEq(lhs.a_b(), rhs.a_b());
}
...
inline bool Eq<DefaultStruct>(const DefaultStruct& lhs,
  ...
  const bool eq_a_b = ArrayEq(lhs.a_b(), rhs.a_b());
  const bool eq_a_i8 = ArrayEq(lhs.a_i8(), rhs.a_i8());
  const bool eq_a_i16 = ArrayEq(lhs.a_i16(), rhs.a_i16());
  const bool eq_a_i32 = ArrayEq(lhs.a_i32(), rhs.a_i32());
  const bool eq_a_i64 = ArrayEq(lhs.a_i64(), rhs.a_i64());
  const bool eq_a_u8 = ArrayEq(lhs.a_u8(), rhs.a_u8());
  const bool eq_a_u16 = ArrayEq(lhs.a_u16(), rhs.a_u16());
  const bool eq_a_u32 = ArrayEq(lhs.a_u32(), rhs.a_u32());
  const bool eq_a_u64 = ArrayEq(lhs.a_u64(), rhs.a_u64());
  const bool eq_a_f = ArrayEq(lhs.a_f(), rhs.a_f());
  const bool eq_a_d = ArrayEq(lhs.a_d(), rhs.a_d());
  const bool eq_a_ei8 = ArrayEq(lhs.a_ei8(), rhs.a_ei8());
  const bool eq_a_ei16 = ArrayEq(lhs.a_ei16(), rhs.a_ei16());
  const bool eq_a_ei32 = ArrayEq(lhs.a_ei32(), rhs.a_ei32());
  const bool eq_a_ei64 = ArrayEq(lhs.a_ei64(), rhs.a_ei64());
  const bool eq_a_eu8 = ArrayEq(lhs.a_eu8(), rhs.a_eu8());
  const bool eq_a_eu16 = ArrayEq(lhs.a_eu16(), rhs.a_eu16());
  const bool eq_a_eu32 = ArrayEq(lhs.a_eu32(), rhs.a_eu32());
  const bool eq_a_eu64 = ArrayEq(lhs.a_eu64(), rhs.a_eu64());
  const bool eq_a_s = ArrayEq(lhs.a_s(), rhs.a_s());
...
TEST(FlatbuffersTableDomainImplTest, CanMutateAnyTableField) {
  ...
  for (size_t i = 0; i < IterationsToHitAll(mutated_fields.size(),
    ...
    mutated_fields["v_t"] |= !Eq(mut->v_str(), init->v_str());
...
TEST(FlatbuffersTableDomainImplTest, MutateSelectedField) {
  ...
  }
  {
    ...
    auto corpus = domain.FromValue(table);
    domain.MutateSelectedField(*corpus, prng, {}, false, 93);
  ...
  }
  {
    ...
    auto corpus = domain.FromValue(table);
    domain.MutateSelectedField(*corpus, prng, {}, false, 94);
  ...
  }
  {
    ...
    auto corpus = domain.FromValue(table);
    domain.MutateSelectedField(*corpus, prng, {}, false, 96);
  ...
  }
  {
    ...
    auto corpus = domain.FromValue(table);
    domain.MutateSelectedField(*corpus, prng, {}, false, 97);
