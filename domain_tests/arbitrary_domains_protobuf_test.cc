...
TEST(ProtocolBuffer,
  ...
  const auto verify_field_changes = [&](absl::string_view name, auto has,
    ...
    while (--iterations > 0 && values.size() < 2) {
      ...
      val.Mutate(domain, bitgen, /*metadata=*/nullptr, false);
    }
...
TEST(ProtocolBuffer,
  ...
  for (int iteration = 0;
    ...
    const auto prev = val;
    val.Mutate(domain, bitgen, /*metadata=*/nullptr, /*only_shrink=*/true);
...
TEST(ProtocolBufferEnum, Arbitrary) {
  ...
  val.Mutate(domain, bitgen, /*metadata=*/nullptr, true);
}
